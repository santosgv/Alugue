...
=========================================
Envia notificações automáticas de WhatsApp:
  - Lembretes de devolução (vence amanhã)
  - Avisos de atraso (passou da data prevista)

Configure no cron para rodar diariamente às 8h:
  0 8 * * * cd /app && python manage.py enviar_whatsapp_diario

Ou no Heroku Scheduler / Railway Cron:
  python manage.py enviar_whatsapp_diario
"""
from django.core.management.base import BaseCommand
from notificacoes.whatsapp_tasks import enviar_lembretes_devolucao, enviar_avisos_atraso


class Command(BaseCommand):
    help = 'Envia lembretes de devolução e avisos de atraso via WhatsApp.'
